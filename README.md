...
This repository is a pnpm workspace monorepo built around the existing Replit artifact structure:
...
- lib/integrations-openai-ai-server — Replit AI Integrations OpenAI client.
...
- Replit AI Integrations for OpenAI access without exposing provider credentials to the client.
...
Configure the environment using .env.example. Database, Clerk, and Replit AI Integration values should be provided through your environment or Replit Secrets; never commit secret values.
...
The artifact workflows provide the ports and proxy routing used by the Replit preview.
...
- Secrets belong in Replit Secrets or environment configuration, not in source control.
